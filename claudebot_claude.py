"""
╔══════════════════════════════════════════════════════════╗
║     CLAUDEBOT v13 — Claude API Version (Anthropic)      ║
║                                                          ║
...
║                                                          ║
║  MODEL: claude-opus-4-5 (screener + analyst)            ║
║  SETUP: pip install anthropic requests ddgs feedparser   ║
║  RUN:   python claudebot_claude.py --single-scan         ║
╚══════════════════════════════════════════════════════════╝
"""
...
SCREENER_MODEL     = "claude-opus-4-5"
ANALYST_MODEL      = "claude-opus-4-5"

# ── Live trading ──────────────────────────────────────────
# Set PAPER_TRADING = False to place real orders.
# Requires POLYMARKET_PRIVATE_KEY to be set.
PAPER_TRADING            = False   # ← flip to False to go live
...
LOG_FILE           = "claudebot_claude_log.json"  # Claude API version — separate log
...
REFLECT_INTERVAL_HOURS = 48   # run self-reflection every 48 hours
MAX_REFLECTIONS_LOAD   = 60   # cap to avoid token overload
...
def telegram_new_trade(trade, state):
    ...
    public_msg = (
        f"🤖 <b>CLAUDEBOT V13 SIGNAL</b>  {badge} <i>{tier_label}</i>\n"
        f"{'─' * 30}\n"
        f"{news_flag}"
        f"<b>{trade['market']}</b>\n\n"
    ...
    if TELEGRAM_PERSONAL_ID:
        private_msg = (
            f"🤖 <b>CLAUDEBOT V13 — PLOYM</b>  {badge} {tier_label}\n"
            f"{'─' * 30}\n"
            f"{news_flag}"
            f"<b>{trade['market']}</b>\n\n"
            f"{pos_emoji} <b>BUY {trade['position']}</b>\n\n"
...
def telegram_daily_summary(state):
    ...
    if TELEGRAM_PERSONAL_ID:
        private_msg = (
            f"📅 <b>CLAUDEBOT V13 DAILY — POLYM</b>\n"
            f"{'─' * 30}\n"
...
def haiku_flag_news(headlines, state):
    ...
    try:
        ...
        raw   = clean_response(resp.content[0].text)
...
# ─────────────────────────────────────────────────────────
#  QWEN RESPONSE HELPERS
# ─────────────────────────────────────────────────────────

def strip_thinking(text):
    """
    Qwen3 thinking models prepend <think>...</think> blocks.
    Strip them so JSON parsing always sees clean output.
    """
    import re as _re
...
def clean_response(raw):
    """Strip thinking tags and code fences from a model response."""
...
"""
    ...
    # ── Post-mortem: ask the model what it got right/wrong ──
    try:
        pm_prompt = (
            f"A prediction market trade just resolved: {outcome}\n\n"
            f"Market: {market}\n"
            f"Original thesis: {research}\n"
            f"Bear case flagged at entry: {bear if bear else 'None'}\n"
            f"Confidence at entry: {conf}% | Edge claimed: {edge}%\n"
            f"Actual outcome: {outcome} | P&L: ${pnl:+.2f}\n\n"
            f"In 3-5 bullet points, answer:\n"
            f"- What was the key signal that drove this trade?\n"
            f"- {'What did the model miss or over-weight? What should have been a red flag?' if not won else 'Was the confidence well-calibrated, or was this lucky? What made the edge real?'}\n"
            f"- Was the bear case useful or was it a missed warning?\n"
            f"- One concrete rule to apply to similar trades in the future.\n\n"
            f"Be specific and direct. Avoid generic platitudes."
        )
        pm_client = anthropic.Anthropic(api_key=ANTHROPIC_API_KEY)
        pm_resp   = pm_client.messages.create(
            model=ANALYST_MODEL,
            max_tokens=500,
            messages=[{"role": "user", "content": pm_prompt}]
        )
        postmortem = clean_response(pm_resp.content[0].text)
        with open(fname, "a", encoding="utf-8") as f:
            f.write(f"\n## Post-Mortem Analysis\n{postmortem}\n")
        log(f"  🧠 Post-mortem appended")
    except Exception as e:
        log(f"  ⚠️  Post-mortem generation failed: {e}")
...
def haiku_screen(markets, state, tier_num):
    ...
    try:
        ...
        # Fallback: if screener returned 0 (Qwen gave no matching IDs),
        # pass through all candidates so the analyst still gets a shot
        if not top_markets and candidates:
            fallback_cap = min(15, tcfg["screener_top_n"])
            log(f"⚠️  Screener returned 0 — passing {fallback_cap} candidates to analyst (fallback cap)")
            return candidates[:fallback_cap]
...
def opus_analyze_short_medium(markets, research, state, tier_num):
    ...
    prompt = (
        ...
        f"CALIBRATION CHECK: Before finalising each confidence score, check the\n"
        f"CONFIDENCE CALIBRATION section in the knowledge graph above. If your\n"
        f"intended band (e.g. 85-90%) shows historical over-confidence, shade it\n"
        f"down. A well-calibrated 80% beats an overconfident 92%.\n\n"
    ...
    try:
        ...
        full_text = strip_thinking(response.content[0].text)
...
def opus_analyze_long(markets, state):
    ...
    prompt = (
        ...
        f"CALIBRATION CHECK: Before finalising each confidence score, check the\n"
        f"CONFIDENCE CALIBRATION section in the knowledge graph (if available).\n"
        f"If that band has historically been overconfident, shade it down.\n\n"
        f"Return ONLY valid JSON array ([] if nothing qualifies):\n"
        f'[{{"market_id":"ID","market":"question","position":"YES or NO",'
...
def _get_clob_client():
    """Return an authenticated ClobClient, or None on failure."""
    ...
    if not POLYMARKET_PRIVATE_KEY:
        ...
        return None
    try:
        # L1 — derive API credentials from private key
        ...
        # L2 — authenticated client for order signing + placement
        # signature_type=0 = EOA (correct for Trust Wallet / external wallets)
        client = ClobClient(
            host=POLYMARKET_HOST,
            key=POLYMARKET_PRIVATE_KEY,
            chain_id=POLYMARKET_CHAIN_ID,
...
def _extract_token_id(mkt, position):
    """Get the CLOB token ID for YES or NO position from a market."""
    ...
    idx = 0 if position == "YES" else 1
    if idx >= len(clob_ids):
        return None
    return str(clob_ids[idx])


def place_live_order(mkt, position, stake, entry_cents):
    """
    Place a real limit buy order on Polymarket CLOB.
    stake       = USDC to spend
    entry_cents = price in cents (e.g. 40 for 40¢)
    ...
    token_id = _extract_token_id(mkt, position)
    if not token_id:
        log(f"❌ No CLOB token ID for {position} on {mkt.get('question','?')[:50]}")
        return None

    price = round(entry_cents / 100, 2)          # 40¢ → 0.40
    size  = round(stake / price, 2)              # shares = USDC / price

    if size < 1.0:
        log(f"  ⏭  Live order too small ({size:.2f} shares)")
        return None

    try:
        log(f"  📡 Placing LIVE order: BUY {size} shares of {position} @ ${price:.2f}")
        ...
        log(f"  ✅ LIVE order placed — ID: {order_id}  status: {status}")
        return {"order_id": order_id, "status": status, "token_id": token_id,
...
def sell_live_position(trade):
    """
    Sell a live position early (for reassessment emergency closes).
    Places a limit sell at current market price minus a small buffer.
    """
    client = _get_clob_client()
    if not client:
        return False

    # Reconstruct market object stub for token extraction
    mkt_stub = {"clobTokenIds": trade.get("clob_token_ids", [])}
    token_id  = trade.get("live_token_id")

    if not token_id:
        log(f"  ⚠️  No token ID stored for live exit — position will resolve at close")
        return False

    shares = round(trade.get("potential_return", 0), 2)   # shares bought at entry
    if shares <= 0:
        return False

    # Fetch current best bid from order book to price the sell
    try:
        ...
        if bids:
            ...
            # No bids — use entry price as fallback (may not fill immediately)
            best_bid = round(trade["entry_price"] / 100, 2)
    except Exception:
        best_bid = round(trade["entry_price"] / 100, 2)

    try:
        log(f"  📡 Placing LIVE sell: {shares} shares @ ${best_bid:.2f}")
        ...
        order_id = resp.get("orderID") or resp.get("id") or str(resp)
        log(f"  ✅ LIVE sell submitted — ID: {order_id}")
        return True
    except Exception as e:
        log(f"  ❌ LIVE sell failed: {e}  (position will resolve at market close)")
        return False
...
def place_paper_trade(rec, markets, state, tier_num, news_triggered=False):
    ...
    if not PAPER_TRADING:
        live_result = place_live_order(mkt, rec["position"], stake, entry)
        if live_result is None:
            log(f"  ❌ Live order failed — trade not recorded")
            state["bankroll"] = round(state["bankroll"] + 0, 2)  # no deduction
            return state
        trade["order_id"]       = live_result["order_id"]
        trade["order_status"]   = live_result["status"]
        trade["live_token_id"]  = live_result["token_id"]
        trade["live_price"]     = live_result["price"]
        trade["live_size"]      = live_result["size"]
    ...
    log(f"  ✅ [{tcfg['label']}] {mode} {flag}BET — {trade['position']} @ {entry}¢  [{tier_label}]")
...
# ─────────────────────────────────────────────────────────
#  SELF-REFLECTION LOOP  (48-hour Qwen meta-learning)
# ─────────────────────────────────────────────────────────

def should_reflect(state):
    """Return True if REFLECT_INTERVAL_HOURS have passed since last reflection."""
    ...
    try:
        ...
        log(f"  🧠 Last reflection: {hours_elapsed:.1f}h ago (threshold: {REFLECT_INTERVAL_HOURS}h)")
        return hours_elapsed >= REFLECT_INTERVAL_HOURS
    except Exception:
        return True


def load_reflections_for_reflect():
    """Load recent trade reflection .md files for self-reflection."""
    import glob as _glob
    if not os.path.exists(REFLECTIONS_DIR):
        return []
    files = sorted(
        _glob.glob(os.path.join(REFLECTIONS_DIR, "*.md")),
...
def build_stats_for_reflect(state):
    """Compute quick performance stats string for the reflection prompt."""
...
def run_self_reflection(state):
    """
    48-hour meta-learning loop.
    Reads trade reflection .md files + live stats, calls Qwen to
    generate a structured GRAPH_REPORT.md, then saves it.
    That report is injected into every future analyst prompt via load_graph_context().
    """
    if not should_reflect(state):
        return state

    reflections = load_reflections_for_reflect()
    if len(reflections) < 3:
        log(f"  ⏭  Self-reflection: only {len(reflections)} reflections — need 3+ to analyse")
        return state

    log(f"  🧠 Running self-reflection ({len(reflections)} reflections)...")
    stats = build_stats_for_reflect(state)
    reflections_text = "\n\n---\n\n".join(reflections)
...
{reflections_text[:12000]}

## Your Task

Write a structured GRAPH_REPORT.md that will be injected into every future analyst prompt. This report should make the bot smarter on the next trade. Be specific and concrete — not generic advice.
...
- [specific pattern generating edge — category, timing, confidence level]

## FAILING PATTERNS (avoid these)
- [specific pattern bleeding money — be concrete about why]
...
- [Top 3 things to look for in the next scan]

## RED FLAGS TO AVOID
- [Specific market types that have repeatedly lost — hard rules]
---"""
    ...
    try:
        ...
        report = clean_response(resp.content[0].text)

        os.makedirs("graphify-out", exist_ok=True)
        with open(GRAPH_REPORT_FILE, "w", encoding="utf-8") as f:
            f.write(report)

        state["last_reflection_utc"] = datetime.now(timezone.utc).isoformat()
        log(f"  ✅ Graph report written ({len(report)} chars) → {GRAPH_REPORT_FILE}")

    except Exception as e:
        log(f"  ⚠️  Self-reflection failed: {e}")
...
def single_scan():
    ...
    # ── Self-reflection loop (every 48h) ─────────────────
    log("── Self-reflection check ────────────────────────────────")
    state = run_self_reflection(state)
    save_state(state)  # persist last_reflection_utc if updated
